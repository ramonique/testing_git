...
data Flags = Flags {
      ...
      optGenDebug        :: Bool,    -- generate extra abstract categories for debugging the grammar
      optGenGrammar      :: Bool {-,
      optNewComp         :: Bool-}
    }
  deriving (Show)
...
--     Option [] ["old-comp"] (NoArg (set $ \o -> o{optNewComp = False})) "Use old trusty compiler.",
     Option [] ["gen-debug"] (NoArg (genDebug True)) "Enable or disable global generation of abstract functions which are used for grammar testing and debugging",
     Option [] ["gen-gram"] (NoArg (genGrammar True)) "Generate partially evaluated grammar for testing",
 ...
 where phase       x = set $ \o -> o { optStopAfterPhase = x }
       ...
       genDebug x = set $ \o -> o { optGenDebug = x }
       genGrammar x = set $ \o -> o { optGenGrammar = x }
